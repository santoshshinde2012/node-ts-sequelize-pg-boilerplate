...
import { Enquiry, EnquiryAttributes, EnquiryCreationAttributes } from "../../../../src/database/models/Enquiry";
...
describe('EnquiryService', () => {
    ...
    it('should return all enquiries', async () => {
        ...
        const sequelizeEnquirys = enquiries.map((org: EnquiryAttributes) => new Enquiry(org));
    ...
    it('should update enquiry with valid input', async () => {
        ...
        const sequelizeEnquiry = new Enquiry(enquiry)
        sequelizeEnquiry.update = jest.fn().mockResolvedValueOnce({ ...enquiry, name: 'Demo' })
    ...
    it('should throw an error if it fails to update as it failed to find the enquiry by id', async () => {
        ...
        await expect(service.update(enquiry.id, { name: 'Demo' })).rejects.toThrow(error);
